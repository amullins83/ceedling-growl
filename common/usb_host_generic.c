    ...
    switch (event)
    {
	    ...
	    case EVENT_TRANSFER:
		{
	        if ( (data != NULL) && (size == sizeof(HOST_TRANSFER_DATA)) )
	        {
	            ...
	            if ( ((HOST_TRANSFER_DATA *)data)->bEndpointAddress == (USB_IN_EP|USB_GENERIC_IN_EP) )
	            {
...
//                	USB_HOST_APP_EVENT_HANDLER(gc_DevData.ID.deviceAddress, EVENT_GENERIC_RX_DONE, &dataCount, sizeof(DWORD) );
	                USB_HOST_APP_EVENT_HANDLER(gc_DevData.ID.deviceAddress, EVENT_GENERIC_RX_DONE, data, sizeof(DWORD) );
	            }
	            else if ( ((HOST_TRANSFER_DATA *)data)->bEndpointAddress == (USB_OUT_EP|USB_GENERIC_OUT_EP) )
	            {
...
BYTE USBHostGenericRead( BYTE deviceAddress, void *buffer, DWORD length )
{
    ...
    gc_DevData.rxLength = 0;
    RetVal = USBHostRead( deviceAddress, USB_IN_EP|USB_GENERIC_IN_EP, (BYTE *)buffer, length );
...
void USBHostGenericTasks( void )
{
    ...
    if (gc_DevData.ID.deviceAddress && gc_DevData.flags.initialized)
    {
        ...
        if (gc_DevData.flags.rxBusy)
        {
            if (USBHostTransferIsComplete( gc_DevData.ID.deviceAddress, USB_IN_EP|USB_GENERIC_IN_EP, &errorCode, &byteCount ))
            {
                gc_DevData.flags.rxBusy = 0;
        ...
        {
            if (USBHostTransferIsComplete( gc_DevData.ID.deviceAddress, USB_OUT_EP|USB_GENERIC_OUT_EP, &errorCode, &byteCount ))
            {
                gc_DevData.flags.txBusy = 0;
...
BYTE USBHostGenericWrite( BYTE deviceAddress, void *buffer, DWORD length )
{
    ...
    RetVal = USBHostWrite( deviceAddress, USB_OUT_EP|USB_GENERIC_OUT_EP, (BYTE *)buffer, length );
    if (RetVal != USB_SUCCESS)
    {
