...
// VID and PID for FTDI chip used in PC interface boards.
const int FTDI_VID = 0x0403ul;        
const int FTDI_PID = 0x6001ul;        
...
void USBHostShutdown( void )
{
    ...
    // Free all extra allocated memory, initialize variables, and reset the state machine.
    USBHostInit( 0 );
	return;
}
        ...
        case STATE_DETACHED:
		{
            switch (usbHostState & SUBSTATE_MASK)
            {
                ...
                case SUBSTATE_TURN_ON_POWER:
				{
                    ...
                    if(USB_HOST_APP_EVENT_HANDLER(USB_ROOT_HUB, EVENT_VBUS_REQUEST_POWER, &powerRequest, sizeof(USB_VBUS_POWER_EVENT_DATA)))
                    {
                        // Power on the module
...
//                            _USB_SetNextState();
                            ...
                            if (pDesc->idVendor == FTDI_VID &&
                                pDesc->idProduct == FTDI_PID )
                            {
                                // Configure FTDI port.
                                _USB_SetNextSubState();
                            }
                ...
                case SUBSTATE_SET_FTDI_RESET:
				{
                    // Reset the FTDI Port 
                    switch (usbHostState & SUBSUBSTATE_MASK)
                    {
                        case SUBSUBSTATE_SEND_RESET_PORT:
						{
                            #ifdef DEBUG_MODE
                                UART2PrintString( "HOST: Reset FTDI Port A.\r\n" );
                            #endif

                            // Set up and send command.
                            pEP0Data[0] = FTDI_SET_REQUEST;
                            pEP0Data[1] = FTDI_REQUEST_RESET;
                            pEP0Data[2] = FTDI_RESET_PURGE_RXTX;
                            pEP0Data[3] = 0;
                            pEP0Data[4] = FTDI_PORTA;
                            pEP0Data[5] = 0;
                            pEP0Data[6] = 0;
                            pEP0Data[7] = 0;
                            _USB_InitControlWrite( usbDeviceInfo.pEndpoint0, pEP0Data, 8, NULL, 0 );
                        ...
                        case SUBSUBSTATE_WAIT_FOR_RESET_PORT:
						{
                            if (usbDeviceInfo.pEndpoint0->status.bfTransferComplete)
                            {
                                if (usbDeviceInfo.pEndpoint0->status.bfTransferSuccessful)
                                {
                                    #ifdef DEBUG_MODE
                                        UART2PrintString( "HOST: Reset FTDI Port A Successful.\r\n" );
                                    #endif
                                    _USB_SetNextSubSubState();
                                }
                                else
                        ...
                        case SUBSUBSTATE_SET_RESET_COMPLETE:
						{
                            // Clean up and advance to the next state.
                            _USB_SetNextSubState();
                            #ifdef DEBUG_MODE
                                UART2PrintString( "HOST: Reset FTDI Port A Complete.\r\n\r\n" );
                            #endif
                            break;
						}
                        default:
                ...
                case SUBSTATE_SET_FTDI_MODEM_CONTROL:
				{
                    // Set the modem control 
                    switch (usbHostState & SUBSUBSTATE_MASK)
                    {
                        case SUBSUBSTATE_SEND_MODEM_CONTROL:
                            #ifdef DEBUG_MODE
                                UART2PrintString( "HOST: Set FTDI Modem Control.\r\n" );
                            #endif

                            // Set up and send command.
                            pEP0Data[0] = FTDI_SET_REQUEST;
                            pEP0Data[1] = FTDI_REQUEST_MODEMCTRL;
                            pEP0Data[2] = FTDI_RTS_ACTIVE;
                            pEP0Data[3] = FTDI_RTS_WRITING;
                            pEP0Data[4] = FTDI_PORTA;
                            pEP0Data[5] = 0;
                            pEP0Data[6] = 0;
                            pEP0Data[7] = 0;
                        ...
                        case SUBSUBSTATE_WAIT_FOR_MODEM_CONTROL:
                            if (usbDeviceInfo.pEndpoint0->status.bfTransferComplete)
                            {
                                if (usbDeviceInfo.pEndpoint0->status.bfTransferSuccessful)
                                {
                                    #ifdef DEBUG_MODE
                                        UART2PrintString( "HOST: Set FTDI Modem Control Successful.\r\n" );
                                    #endif
                                    _USB_SetNextSubSubState();
                                }
                                else
                        ...
                        case SUBSUBSTATE_SET_MODEM_CONTROL_COMPLETE:
                            // Clean up and advance to the next state.
                            _USB_SetNextSubState();
                            #ifdef DEBUG_MODE
                                UART2PrintString( "HOST: Set FTDI Modem Control Complete.\r\n\r\n" );
                            #endif
                            break;
                ...
                case SUBSTATE_SET_FTDI_FLOW_CONTROL:
				{
                    // Set the flow control 
                    switch (usbHostState & SUBSUBSTATE_MASK)
                    {
                        case SUBSUBSTATE_SEND_FLOW_CONTROL:
                            #ifdef DEBUG_MODE
                                UART2PrintString( "HOST: Set FTDI Flow Control.\r\n" );
                            #endif

                            // Set up and send command.
                            pEP0Data[0] = FTDI_SET_REQUEST;
                            pEP0Data[1] = FTDI_REQUEST_SETFLOWCTRL;
                            pEP0Data[2] = 0;
                            pEP0Data[3] = 0;
                            pEP0Data[4] = FTDI_PORTA;
                            pEP0Data[5] = FTDI_RTSCTS_FLOWCTRL;
                            pEP0Data[6] = 0;
                            pEP0Data[7] = 0;
                            _USB_InitControlWrite( usbDeviceInfo.pEndpoint0, pEP0Data, 8, NULL, 0 );
                            _USB_SetNextSubSubState();
                        ...
                        case SUBSUBSTATE_WAIT_FOR_FLOW_CONTROL:
                            if (usbDeviceInfo.pEndpoint0->status.bfTransferComplete)
                            {
                                if (usbDeviceInfo.pEndpoint0->status.bfTransferSuccessful)
                                {
                                    #ifdef DEBUG_MODE
                                        UART2PrintString( "HOST: Set FTDI Flow Control Successful.\r\n" );
                                    #endif
                                    _USB_SetNextSubSubState();
                                }
                                else
                        ...
                        case SUBSUBSTATE_SET_FLOW_CONTROL_COMPLETE:
                            // Clean up and advance to the next state.
                            _USB_SetNextSubState();
                            #ifdef DEBUG_MODE
                                UART2PrintString( "HOST: Set FTDI Flow Control Complete.\r\n\r\n" );
                            #endif
                            break;
                ...
                case SUBSTATE_SET_FTDI_BAUDRATE:
				{
                    // Set the baud rate. 
                    switch (usbHostState & SUBSUBSTATE_MASK)
                    {
                        case SUBSUBSTATE_SEND_BAUDRATE:
						{
                            #ifdef DEBUG_MODE
                                UART2PrintString( "HOST: Set FTDI Baud Rate.\r\n" );
                            #endif

                            // Set up and send command.
                            pEP0Data[0] = FTDI_SET_REQUEST;
                            pEP0Data[1] = FTDI_REQUEST_SETBAUDRATE;
                            pEP0Data[2] = 0x06;       																						// 460800 (461538 actual).
                            pEP0Data[3] = 0x40;
//                          pEP0Data[2] = 0x4E;				       																			// 38400 (38400 actual)
//                          pEP0Data[3] = 0xC0;
                            pEP0Data[4] = FTDI_PORTA;
                            pEP0Data[5] = 0x00;
                            pEP0Data[6] = 0x00;
                        ...
                        case SUBSUBSTATE_WAIT_FOR_BAUDRATE:
						{
                            if (usbDeviceInfo.pEndpoint0->status.bfTransferComplete)
                            {
                                if (usbDeviceInfo.pEndpoint0->status.bfTransferSuccessful)
                                {
                                    #ifdef DEBUG_MODE
                                        UART2PrintString( "HOST: Set FTDI Baud Rate Successful.\r\n" );
                                    #endif
                                    _USB_SetNextSubSubState();
                                }
                                else
                        ...
                        case SUBSUBSTATE_SET_BAUDRATE_COMPLETE:
						{
                            // Clean up and advance to the next state.
                            _USB_SetNextSubState();
                            #ifdef DEBUG_MODE
                                UART2PrintString( "HOST: Set FTDI Baud Rate Complete.\r\n\r\n" );
                            #endif
                            break;
						}
                        default:
                ...
                case SUBSTATE_SET_FTDI_DATA:
				{
                    // Set the data settings. 
                    switch (usbHostState & SUBSUBSTATE_MASK)
                    {
                        case SUBSUBSTATE_SEND_DATA:
						{
                            #ifdef DEBUG_MODE
                                UART2PrintString( "HOST: Set FTDI Data Characteristics.\r\n" );
                            #endif

                            // Set up and send command.
                            pEP0Data[0] = FTDI_SET_REQUEST;
                            pEP0Data[1] = FTDI_REQUEST_SETDATA;
                            pEP0Data[2] = FTDI_EIGHT_BITS;
                            pEP0Data[3] = FTDI_PARITY_NONE | FTDI_STOP_ONE | FTDI_BREAK_NO;
                            pEP0Data[4] = FTDI_PORTA;
                            pEP0Data[5] = 0;
                            pEP0Data[6] = 0;
                            pEP0Data[7] = 0;
                        ...
                        case SUBSUBSTATE_WAIT_FOR_DATA:
						{
                            if (usbDeviceInfo.pEndpoint0->status.bfTransferComplete)
                            {
                                if (usbDeviceInfo.pEndpoint0->status.bfTransferSuccessful)
                                {
                                    #ifdef DEBUG_MODE
                                        UART2PrintString( "HOST: Set FTDI Data Characteristics Successful.\r\n" );
                                    #endif
                                    _USB_SetNextSubSubState();
                                }
                                else
                        ...
                        case SUBSUBSTATE_SET_DATA_COMPLETE:
						{
                            // Clean up and advance to the next state.
                            _USB_InitErrorCounters();
                            _USB_SetNextSubState();
                            ...
                            #ifdef DEBUG_MODE
                                UART2PrintString( "HOST: Set FTDI Data Characteristics Complete.\r\n\r\n" );
                            #endif
                            break;
						}
                        default:
                ...
                case SUBSTATE_SET_FTDI_SETBITMODE:
				{
                    // Set the bit mode settings. 
                    switch (usbHostState & SUBSUBSTATE_MASK)
                    {
                        case SUBSUBSTATE_SEND_SETBITMODE:
						{
                            #ifdef DEBUG_MODE
                                UART2PrintString( "HOST: Set FTDI Bit Mode.\r\n" );
                            #endif

                            // Set up and send command.
                            pEP0Data[0] = FTDI_SET_REQUEST;
                            pEP0Data[1] = FTDI_REQUEST_SETBITMODE;
                            pEP0Data[2] = 0x00;
                            pEP0Data[3] = FTDI_MODE_OFF;
                            pEP0Data[4] = FTDI_PORTA;
                            pEP0Data[5] = 0;
                            pEP0Data[6] = 0;
                            pEP0Data[7] = 0;
                        ...
                        case SUBSUBSTATE_WAIT_FOR_SETBITMODE:
						{
                            if (usbDeviceInfo.pEndpoint0->status.bfTransferComplete)
                            {
                                if (usbDeviceInfo.pEndpoint0->status.bfTransferSuccessful)
                                {
                                    #ifdef DEBUG_MODE
                                        UART2PrintString( "HOST: Set FTDI Bit Mode Successful.\r\n" );
                                    #endif
                                    _USB_SetNextSubSubState();
                                }
                                else
                        ...
                        case SUBSUBSTATE_SETBITMODE_COMPLETE:
						{
                            // Clean up and advance to the next state.
                            _USB_InitErrorCounters();
                            _USB_SetNextState();
                            
                            // Acknowledge FTDI is connected.
                            App_State_FTDI = DEVICE_CONNECTED;
                            #ifdef DEBUG_MODE
                                UART2PrintString( "HOST: Set FTDI Bit Mode Complete.\r\n\r\n" );
                            #endif
                            break;
						}
                        default:
        ...
        (!(U1IEbits.UERRIE && U1IRbits.UERRIF) || (pCurrentEndpoint->bmAttributes.bfTransferType == USB_TRANSFER_TYPE_ISOCHRONOUS)))
    {
        ...
        if ((pCurrentEndpoint->bEndpointAddress & 0x0F) == (U1TOK & 0x0F))
        {
            ...
            if (pBDT->STAT.PID == PID_ACK)
            {
                ...
                {
                    if (pCurrentEndpoint->bmAttributes.bfTransferType == USB_TRANSFER_TYPE_BULK) 
                    {
                        if (pCurrentEndpoint->dataCount < pCurrentEndpoint->dataCountMax) 
                        {
                        }    
                    }
