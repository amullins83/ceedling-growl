...
//U2BRG register value and baudrate mistake calculation
	...
	#define BAUDRATEREG2_72MHz  ((GetPeripheralClock_72MHz()+(BRG_DIV2/2*BAUDRATE2))/BRG_DIV2/BAUDRATE2-1)							// (9DP v1.02.00)
	#define BAUDRATEREG2_80MHz  ((GetPeripheralClock_80MHz()+(BRG_DIV2/2*BAUDRATE2))/BRG_DIV2/BAUDRATE2-1)							// (9DP v1.02.00)
#else
	...
	#define BAUD_ACTUAL_72MHz   (GetPeripheralClock_72MHz()/BRG_DIV2/(BAUDRATEREG2_72MHz+1))										// (9DP v1.02.00)
	#define BAUD_ACTUAL_80MHz	(GetPeripheralClock_80MHz()/BRG_DIV2/(BAUDRATEREG2_80MHz+1))										// (9DP v1.02.00)
#else
...
#define BAUD_ERROR_72MHz		((BAUD_ACTUAL_72MHz > BAUDRATE2) ? BAUD_ACTUAL_72MHz-BAUDRATE2 : BAUDRATE2-BAUD_ACTUAL_72MHz)		// (9DP v1.02.00)
#define BAUD_ERROR_80MHz		((BAUD_ACTUAL_80MHz > BAUDRATE2) ? BAUD_ACTUAL_80MHz-BAUDRATE2 : BAUDRATE2-BAUD_ACTUAL_80MHz)		// (9DP v1.02.00)
...
//	errorPercent = ((BAUD_ERROR*100+BAUDRATE2/2)/BAUDRATE2);										// (hidden 9DP v1.02.00)

	configRegister = DEVCFG2 & 0x00000070;															// (9DP v1.02.00)
	if(configRegister == 0x00000030)																// FPLLMULT = 3 [18x] (9DP v1.02.00)
	{
		errorPercent = ((BAUD_ERROR_72MHz*100+BAUDRATE2/2)/BAUDRATE2);								// (9DP v1.02.00)
	}
	else																							// (9DP v1.02.00)
	{
		errorPercent = ((BAUD_ERROR_80MHz*100+BAUDRATE2/2)/BAUDRATE2);								// (9DP v1.02.00)
	}
...
//	U2BRG = BAUDRATEREG2;																			// hidden (9DP v1.02.00)

	configRegister = DEVCFG2 & 0x00000070;															// (9DP v1.02.00)
	if(configRegister == 0x00000030)																// FPLLMULT = 3 [18x] (9DP v1.02.00)
	{
	    U2BRG = BAUDRATEREG2_72MHz;																	// (9DP v1.02.00)
	}
	else																							// (9DP v1.02.00)
	{
	    U2BRG = BAUDRATEREG2_80MHz;																	// (9DP v1.02.00)
	}
