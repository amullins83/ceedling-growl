...
#elif defined(_SPI2A)
	#define	_SPI_DEF_CHN_		2A
#elif defined(_SPI1A)
	#define	_SPI_DEF_CHN_		1A
#elif defined(_SPI3A)
	#define	_SPI_DEF_CHN_		3A
...
typedef enum
{
	...
	SPI_CHANNEL2	= 2,
	SPI_CHANNEL2A	= 2,	// alternate name for SPI2
	SPI_CHANNELS	= SPI_CHANNELS+1,
#endif	// _SPI2 || _SPI2A
 ...
 *                  by the user software (or the FIFO is full, in ebhanced buffer mode), then SPIROV bit will be set.The 
 *                  module will not transfer the received data from SPIxSR to the SPIxRXB (FIFO). 
 *                  Further data reception is disabled until the SPIROV bit is cleared. 
 *                  The SPIROV bit is not cleared automatically by the module and must be 
 *                  cleared by the user software.
 *
