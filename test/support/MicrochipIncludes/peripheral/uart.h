...
typedef enum
{
    ...
    // Legacy UART2 equivalency.
    UART3A = UART2,
#endif

#ifdef _UART1B
    // UART1B Module ID (not available if UART1A CTS/RTS used).
    UART1B,
#endif
#ifdef _UART2A
    // UART2A Module ID.
...
#endif
#ifdef _UART2B
    // UART2B Module ID (not available if UART2A CTS/RTS used).
    UART2B,
#endif
#ifdef _UART3B
    // UART3B Module ID (not available if UART3A CTS/RTS used).
    UART3B,
#endif
...
/* UART Enable Helper Macro
  ...
  Example:
    <code>
    UARTEnable(UART1A, UART_ENABLE_FLAGS(UART_PERIPHERAL | UART_RX));
    </code>
...
/* UART Disable Helper Macro
  ...
  Example:
    <code>
    UARTEnable(UART1A, UART_DISABLE_FLAGS(UART_TX));
    </code>
...
/*******************************************************************************
  ...
  Example:
    <code>
    UARTEnable(UART1A, UART_ENABLE_FLAGS(UART_PERIPHERAL | UART_RX | UART_TX));
    </code>
...
/*******************************************************************************
  ...
  Example:
    <code>
    if ( UART_SUCCESS != UARTConfigure(UART1A, UART_RTS_WHEN_RX_NOT_FULL|UART_ENABLE_PINS_CTS_RTS) )
    {
        DBPRINT("UART1A configuration error!\n");
    }
    </code>
...
/*******************************************************************************
  ...
  Example:
    <code>
    if ( != UARTSetFifoMode(UART1A, UART_INTERRUPT_ON_TX_DONE|UART_INTERRUPT_ON_RX_NOT_EMPTY) )
    {
        DBPRINT("UART1A invalid FIFO mode!\n");
    }
    </code>
...
/*******************************************************************************
  ...
  Example:
    ...
    actualDataRate = UARTSetDataRate(UART1A, GetPeripheralClock(), DESIRED_DATA_RATE);
    </code>
...
/*******************************************************************************
  ...
  Example:
    <code>
    currentRate = UARTGetDataRate(UART1A, GetPeripheralClock());
    if ( abs(currentRate-UART1A_DESIRED_DATA_RATE) > (UART1A_DESIRED_DATA_RATE/10) )
    {
        DBPRINT("UART1A data rate (%d) error exceeds 10\%\n", currentRate);
    }
    </code>

  Remarks:
    The current (actual) the data rate selected may be slightly different than bit 
...
/*******************************************************************************
  ...
  Example:
    <code>
    UARTSetLineControl(UART_DATA_SIZE_8_BITS|UART_PARITY_NONE|UART_STOP_BITS_1);
    </code>
...
/*******************************************************************************
  ...
  Example:
    <code>
    lineStatus = UARTGetLineStatus(UART1A);
...
{
    return uartReg[id]->sta.reg & (UART_LINE_STATUS_MASK);
}
...
{
    return (!(uartReg[id]->sta.reg & _U1STA_UTXBF_MASK));
}
...
/*******************************************************************************
  ...
  Precondition:
    The UART moduel should have been appropriately configured.  The configuration must
...
/*******************************************************************************
  ...
  Example:
    <code>
    if (UARTTransmissionHasCompleted(UART1A))
    {
...
{
    return (_U1STA_TRMT_MASK == (uartReg[id]->sta.reg & _U1STA_TRMT_MASK));
}
...
/*******************************************************************************
  ...
  Precondition:
    The UART module must have been configured appropriately and enabled and a complete
    data frame must have been sent to the UART.
...
{
    return (_U1STA_URXDA_MASK == (uartReg[id]->sta.reg & _U1STA_URXDA_MASK));
}
...
/*******************************************************************************
  ...
  Description:
    This routine allows the caller to read a data received by the UART.
...
/*******************************************************************************
  ...
  Example:
    <code>
    UARTSetAddress(UART1A, IEEE_485_SLAVE_ADDRESS, TRUE);
    </code>
...
/*******************************************************************************
  ...
  Example:
    ...
    UARTSetAddress(UART1A, SLAVE_ADDRESS, FALSE);
    
    // ...
    
    // Ignore all UART traffic until we are addressed
    UARTWatchForAddress(UART1A, TRUE);
    </code>
...
/*******************************************************************************
  ...
  Example:
    ...
    {
        UARTStartAutoDataRateDetect (UART1A);
    }
    </code>
...
/*******************************************************************************
  Function:
    void UARTDataRateDetected ( UART_MODULE id )
  ...
  Example:
    <code>
    if (UARTDataRateDetected(UART1A))
    {
        currentRate = UARTGetDataRate(UART1A, GetPeripheralClock());
    }
    </code>
  ...
  Remarks:
    This routine only returns FALSE for a short period after automatic rate 
    detection has been started (during the period that it is ongoing).  At
    other times, it returns TRUE.
  *****************************************************************************/

extern inline BOOL __attribute__((always_inline)) UARTDataRateDetected ( UART_MODULE id )
{
    return (!(uartReg[id]->mode.reg & _U1MODE_ABAUD_MASK));    
}
